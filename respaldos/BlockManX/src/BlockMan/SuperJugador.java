...
public class SuperJugador implements KeyListener {
    ...
    //limite dx incremental es el doble cuando se corre.
    public int potencia_wall_jump_x=10,limit_dx_incremental=8,dx_wall_jump,x,x_anterior,y,inicioX,inicioY,dx,dy,limite,jugador,subX,subY,posicion,posicionAnterior,posicionCaminando,incremento,incrementoCorriendo,incrementoAire,incrementoSuelo,fase_salto,posCamX,posCamY,nadando,quemandose,curandose,direccion;//,cambiar_y
    ...
    public boolean espacio,arriba,izquierda,derecha,agacharse,disparo,cambiar,corriendo,disparando,disparado,cambiado,saltando,saltando2,deslizando,skineado;
    ...
    public boolean dx_incremental=true;//acelerar gradualmente en el eje x.
    
    public int min_dy_wall_jump=-2;//delta y minimo para volver a saltar en una pared (al caer).
    
    
    public SuperJugador(int x, int y, int jugador){
        
        this.megaman=Juego.megaman;
    ...
    //                                    }
                            ...
                            if(!instantaneo){
                                centro=MapaTiles.mapaFisico[quad_y][quad_x];
                                //comprobar estrella:
    ...
    public void reaparecer(){
        ...
        if(this.checkPointIndex!=0){
            this.x=MapaTiles.punto.get(this.checkPointIndex-1).posicionPuntoX-(2*Tile.TILE_WIDTH);
            this.y=MapaTiles.punto.get(this.checkPointIndex-1).posicionPuntoY-(2*Tile.TILE_HEIGHT);
    ...
    public boolean bloqueBloqueado(int x, int y, int indice, boolean corregir){
        ...
        for(int i=1;i<=4;i++){
            ...
            if(direccion==1){
                espacio1=(quad_x-1>0 && quad_y-1>0)?MapaTiles.mapaFisico[quad_y-1][quad_x-1]:1;
                espacio2=(quad_x-1>0)?MapaTiles.mapaFisico[quad_y][quad_x-1]:1;
                espacio3=(quad_x-1>0 && quad_y+1<Juego.alto)?MapaTiles.mapaFisico[quad_y+1][quad_x-1]:1;
            ...
            if(direccion==2){
                espacio1=(quad_x+1<Juego.ancho && quad_y-1>0)?MapaTiles.mapaFisico[quad_y-1][quad_x+1]:1;
                espacio2=(quad_x+1<Juego.ancho)?MapaTiles.mapaFisico[quad_y][quad_x+1]:1;
                espacio3=(quad_x+1<Juego.ancho && quad_y+1<Juego.alto)?MapaTiles.mapaFisico[quad_y+1][quad_x+1]:1;
                if(pos_quad_x>(tile.TILE_WIDTH/2)){
                    desfaseX=-1;
                    if(pos_quad_y==(tile.TILE_HEIGHT/2)){
                        if(espacio2==1){
            ...
            if(direccion==3){
                espacio1=(quad_x-1>0 && quad_y-1>0)?MapaTiles.mapaFisico[quad_y-1][quad_x-1]:1;
                espacio2=((quad_y-1)>0)?MapaTiles.mapaFisico[quad_y-1][quad_x]:1;
                espacio3=(quad_x+1<Juego.ancho && quad_y-1>0)?MapaTiles.mapaFisico[quad_y-1][quad_x+1]:1;
                if(pos_quad_y<(tile.TILE_HEIGHT/2)){
                    desfaseY=-1;
                    if(pos_quad_x==(tile.TILE_WIDTH/2)){
                        if(espacio2==1){
            ...
            if(direccion==4){
                espacio1=(quad_x-1>0 && quad_y+1<Juego.alto)?MapaTiles.mapaFisico[quad_y+1][quad_x-1]:1;
                espacio2=(quad_y+1<Juego.alto)?MapaTiles.mapaFisico[quad_y+1][quad_x]:1;
                espacio3=(quad_x+1<Juego.ancho && quad_y+1<Juego.alto)?MapaTiles.mapaFisico[quad_y+1][quad_x+1]:1;
                if(pos_quad_y>(tile.TILE_HEIGHT/2)){
                    desfaseY=1;
                    if(pos_quad_x==(tile.TILE_WIDTH/2)){
    ...
    public void actualizar(){
        ...
        if(this.bounce || this.dx_incremental) incremento=this.incrementoSuelo;
        if((this.nadando>0 || this.curandose>0 || this.quemandose>0) && !this.bounce && !this.dx_incremental) incremento/=2;//se mueve a la mitad de la velocidad en el eje x.
        ...
        if((this.bloqueado_abajo || ((!this.derecha && !this.izquierda)) || this.dx_wall_jump!=0) && !this.bounce && !this.dx_incremental){//resetear el dx...
            this.dx=0;
        ...
        if(((this.derecha || this.izquierda || this.deslizando) && !(this.derecha && this.izquierda)) || this.dx_wall_jump!=0){
            ...
            while(i<veces){
                ...
                if(this.izquierda==true || (this.deslizando && this.direccion==1)){
                    this.direccion=1;
                    if((!this.fase_completa==true || !this.retraso) && bloqueado_izquierda==false) if(this.bounce || this.dx_incremental) this.dx-=incrementoSuelo; else this.dx=-incremento;
                    if(!bloqueado_abajo){
                ...
                if(this.derecha==true || (this.deslizando && this.direccion==2)){
                    this.direccion=2;
                    if((!this.fase_completa==true || !this.retraso) && bloqueado_derecha==false) if(this.bounce || this.dx_incremental) this.dx+=this.incrementoSuelo; else this.dx=incremento;
                    if(!bloqueado_abajo){
            ...
            if(this.nadando>0 || this.quemandose>0 || this.curandose>0){
                if(this.bounce || this.dx_incremental){
                    if(this.dx>0){
                        this.dx-=1;
                    }
                    if(this.dx<0){
        ...
        if(this.bounce || this.dx_incremental){
            //mitad de velocidad en agua:
            int limit_dx_incremental=this.limit_dx_incremental;
            if(this.dx>limit_dx_incremental){
                this.dx=limit_dx_incremental;
            }
            if(this.dx<-limit_dx_incremental){
                this.dx=-limit_dx_incremental;
            }
            if(this.nadando>0 || this.quemandose>0 || this.curandose>0){
                limit_dx_incremental=this.limit_dx_incremental/2;
                if(this.dx>limit_dx_incremental){
                    this.dx-=2;
                }
                if(this.dx<-limit_dx_incremental){
            ...
            if(bloqueado_derecha && this.dx>0){
                if(this.bounce) this.dx=-Math.abs(this.dx)+((this.derecha || this.izquierda)?0:1); else this.dx=0;
            }
            if(bloqueado_izquierda && this.dx<0){
                if(this.bounce) this.dx=Math.abs(this.dx)-((this.derecha || this.izquierda)?0:1); else this.dx=0;
            }
            int signo=(this.dx<0?-1:(this.dx>0?1:0));
            if((bloqueado_abajo || bloqueado_arriba) && !(this.derecha || this.izquierda)) this.dx-=(this.bounce?signo:signo*1);//mayor roce cuando es sin bounce.
            if(this.dx>Tile.TILE_WIDTH*seccionadorX/2 || this.dx<-Tile.TILE_WIDTH*seccionadorX/2){
                int dxaux=this.dx;
                this.subX+=Tile.TILE_WIDTH*seccionadorX/2*signo;
                this.x=subX/seccionadorX;
                this.bloqueado_izquierda=bloqueado(1,true,false);
                this.bloqueado_derecha=bloqueado(2,true,false);
                dxaux-=Tile.TILE_WIDTH*seccionadorX/2*signo;
                if(bloqueado_derecha && this.dx!=0){
                    if(this.bounce) this.dx=-Math.abs(this.dx)+((this.derecha || this.izquierda)?0:1); else this.dx=0;//this.dx=-Math.abs(this.dx)+((this.derecha || this.izquierda)?0:1);
                    if(this.bounce) dxaux=-Math.abs(dxaux)+((this.derecha || this.izquierda)?0:1); else dxaux=0;//dxaux=-Math.abs(dxaux)+((this.derecha || this.izquierda)?0:1);
                }
                if(bloqueado_izquierda && this.dx!=0){
                    if(this.bounce) this.dx=Math.abs(this.dx)-((this.derecha || this.izquierda)?0:1); else this.dx=0;//this.dx=Math.abs(this.dx)-((this.derecha || this.izquierda)?0:1);
                    if(this.bounce) dxaux=Math.abs(dxaux)-((this.derecha || this.izquierda)?0:1); else dxaux=0;//dxaux=Math.abs(dxaux)-((this.derecha || this.izquierda)?0:1);
            ...
            switch(this.arma_actual){
                case "bullet":
                    if(Juego.online){
                        ...
                        Juego.infoElementos+="1,"+this.direccion+","+Juego.superJugador.get(jugador).x+Juego.superJugador.get(jugador).positionParts[Integer.parseInt(Juego.superJugador.get(jugador).brazoDer)][0][1]+","+Juego.superJugador.get(jugador).x+Juego.superJugador.get(jugador).positionParts[Integer.parseInt(Juego.superJugador.get(jugador).brazoDer)][1][1]+";";
                    }else{
                        Juego.weaponBullet.add(new WeaponBullet(this.jugador,1));
                    }
                    this.energy=0;
                    break;
                case "axe":
                    if(this.axeMunition>0){
                        if(Juego.online){
                            ...
                            Juego.infoElementos+="2,"+this.direccion+","+Juego.superJugador.get(jugador).x+Juego.superJugador.get(jugador).positionParts[Integer.parseInt(Juego.superJugador.get(jugador).brazoDer)][0][1]+","+Juego.superJugador.get(jugador).x+Juego.superJugador.get(jugador).positionParts[Integer.parseInt(Juego.superJugador.get(jugador).brazoDer)][1][1]+";";
                        }else{
                            Juego.weaponAxe.add(new WeaponAxe(this.jugador,1));
                        }
                ...
                case "ball":
                    if(this.ballMunition>0){
                        if(Juego.online){
                            ...
                            Juego.infoElementos+="3,"+this.direccion+","+Juego.superJugador.get(jugador).x+Juego.superJugador.get(jugador).positionParts[Integer.parseInt(Juego.superJugador.get(jugador).brazoDer)][0][1]+","+Juego.superJugador.get(jugador).x+Juego.superJugador.get(jugador).positionParts[Integer.parseInt(Juego.superJugador.get(jugador).brazoDer)][1][1]+";";
                        }else{
                            Juego.weaponBall.add(new WeaponBall(this.x+this.positionParts[Integer.parseInt(this.brazoDer)][0][1],this.y+this.positionParts[Integer.parseInt(this.brazoDer)][1][1],this.jugador));
                        }
    ...
    */
    ...
    public boolean bloqueado(int direccion, boolean corregir, boolean correccionMinima){
        ...
        for(int i=0;i<(this.positionParts.length-1);i++){
            ...
            if(x>(tile.TILE_WIDTH*(Juego.ancho-1)*seccionadorX)){
                this.subX-=(x-(tile.TILE_WIDTH*(Juego.ancho-1)*seccionadorX));
            }
            if(y<0){
                this.subY-=y;
            }
            if(y>(tile.TILE_HEIGHT*(Juego.alto-1)*seccionadorY)){
                this.subY-=(y-(tile.TILE_HEIGHT*(Juego.alto-1)*seccionadorY));
            }
            ...
            int centro=MapaTiles.mapaFisico[quad_y][quad_x];
            if(centro==1){
            ...
            if(direccion==1){//izquierda
                ...
                espacio2=(quad_x-1>=0)?MapaTiles.mapaFisico[quad_y][quad_x-1]:1;
                espacio3=(quad_x-1>=0 && quad_y+1<Juego.alto)?MapaTiles.mapaFisico[quad_y+1][quad_x-1]:1;
                espacio4=(quad_y+1<Juego.alto)?MapaTiles.mapaFisico[quad_y+1][quad_x]:1;
                espacio5=(quad_y-1>=0)?MapaTiles.mapaFisico[quad_y-1][quad_x]:1;
                //espacio5=0;
                //espacio4=0;
                if(pos_quad_x<=(Tile.TILE_WIDTH/2)){
            ...
            if(direccion==3){//arriba
                ...
                espacio2=(quad_y-1>=0)?MapaTiles.mapaFisico[quad_y-1][quad_x]:1;
                espacio3=(quad_x+1<Juego.ancho && quad_y-1>=0)?MapaTiles.mapaFisico[quad_y-1][quad_x+1]:1;
                espacio4=(quad_x-1>=0)?MapaTiles.mapaFisico[quad_y][quad_x-1]:1;
                espacio5=(quad_x+1<Juego.ancho)?MapaTiles.mapaFisico[quad_y][quad_x+1]:1;
                espacio5=0;
                espacio4=0;
                if(pos_quad_y<=(Tile.TILE_HEIGHT/2)){

...
public class WeaponBall {
    ...
    public WeaponBall(int x, int y, int jugador){
        this.inicioX=x+(ServerJuego.superJugador.get(jugador).direccion==1?-1:1);
        this.inicioY=y;
        ...
        this.x=x+(ServerJuego.superJugador.get(jugador).direccion==1?-1:1);
        this.y=y;
        this.subX=x*seccionador+(ServerJuego.superJugador.get(jugador).direccion==1?-1:1)*this.seccionador;
        this.subY=y*seccionador;
        this.dx=(ServerJuego.superJugador.get(jugador).x-ServerJuego.superJugador.get(jugador).x_anterior)*Tile.TILE_WIDTH;
        if(this.dx==0){
            this.dx=(ServerJuego.superJugador.get(jugador).direccion==1?-1:1)*this.seccionador;
        }
        this.dy=0;
    ...
    public void updateBall(){
        if(this.x+Tile.TILE_WIDTH>ServerJuego.ancho*Tile.TILE_WIDTH){
            this.x=ServerJuego.ancho*Tile.TILE_WIDTH-Tile.TILE_WIDTH;
            this.subX=(ServerJuego.ancho*Tile.TILE_WIDTH-Tile.TILE_WIDTH)*seccionador;
            this.dx=-dx;
        }
        if(this.y+Tile.TILE_HEIGHT>ServerJuego.alto*Tile.TILE_HEIGHT){
            this.y=ServerJuego.alto*Tile.TILE_HEIGHT-Tile.TILE_HEIGHT;
            this.subY=(ServerJuego.alto*Tile.TILE_HEIGHT-Tile.TILE_HEIGHT)*seccionador;
            this.dy=-dy;
        }
        if(this.x<0){
            this.x=0;
                ...
                if((ServerJuego.superJugador.get(i).x+ServerJuego.superJugador.get(i).positionParts[j][0][1])<(this.x+Tile.TILE_WIDTH)
                    ...
                    if(this.jugador!=i){
                        ...
                        if(this.health<=0){
                            ...
                            this.broken=true;
                            ServerJuego.superJugador.get(i).changeHealth(-damage);
                            return;
                        }else{
                            ServerJuego.superJugador.get(i).changeHealth(-damage);
                        }
                    }
                    int dx_jugador=(ServerJuego.superJugador.get(i).x-ServerJuego.superJugador.get(i).x_anterior)*seccionador*Tile.TILE_WIDTH/4;
                    int m=1;
                    if(dx_jugador<0){
                        m=-1;
                    }
                    ...
                    //this.dx=-this.dx;
                    if((this.x)<(ServerJuego.superJugador.get(i).x+ServerJuego.superJugador.get(i).positionParts[Integer.parseInt(ServerJuego.superJugador.get(i).cabeza)][0][1])){
        ...
        for(int i=0;i<ServerJuego.superEnemigo.size();i++){
            for(int j=1;j<ServerJuego.superEnemigo.get(i).positionParts.length;j++){
                if(!ServerJuego.superEnemigo.get(i).deadDimension){
                    if((ServerJuego.superEnemigo.get(i).x+ServerJuego.superEnemigo.get(i).positionParts[j][0][1])<(this.x+Tile.TILE_WIDTH)
                        ...
                        if(this.health<=0){
                            damage+=health;//si se pasa se le quita.
                            this.broken=true;
                            ServerJuego.superEnemigo.get(i).changeHealth(-damage);
                            return;
                        }else{
                            ServerJuego.superEnemigo.get(i).changeHealth(-damage);
                        }

                        int dx_jugador=(ServerJuego.superEnemigo.get(i).x-ServerJuego.superEnemigo.get(i).x_anterior)*seccionador*Tile.TILE_WIDTH/4;
                        int m=1;
                        if(dx_jugador<0){
                            m=-1;
                        }
                        if(dx_jugador!=0) dx_jugador=m*((int)(Math.random()*dx_jugador+1));
                        //this.dx=-this.dx;
                        if((ServerJuego.superEnemigo.get(i).x+ServerJuego.superEnemigo.get(i).positionParts[Integer.parseInt(ServerJuego.superEnemigo.get(i).cabeza)][0][1])>=(this.x)){
                            this.dx=-(Math.abs(this.dx)-dx_jugador);
                        }else{
                            this.dx=(Math.abs(this.dx)+dx_jugador);
                        }
        ...
        espacios[0][0]=(quad_x-1>0 && quad_y-1>0)?MapaServer.mapaFisico[quad_y-1][quad_x-1]:1;
        espacios[0][1]=((quad_y-1)>0)?MapaServer.mapaFisico[quad_y-1][quad_x]:1;
        espacios[0][2]=(quad_x+1<ServerJuego.ancho && quad_y-1>0)?MapaServer.mapaFisico[quad_y-1][quad_x+1]:1;
        espacios[1][0]=(quad_x-1>0)?MapaServer.mapaFisico[quad_y][quad_x-1]:1;
        espacios[1][1]=MapaServer.mapaFisico[quad_y][quad_x];
        espacios[1][2]=(quad_x+1<ServerJuego.ancho)?MapaServer.mapaFisico[quad_y][quad_x+1]:1;
        espacios[2][0]=(quad_x-1>0 && quad_y+1<ServerJuego.alto)?MapaServer.mapaFisico[quad_y+1][quad_x-1]:1;
        espacios[2][1]=(quad_y+1<ServerJuego.alto)?MapaServer.mapaFisico[quad_y+1][quad_x]:1;
        espacios[2][2]=(quad_x+1<ServerJuego.ancho && quad_y+1<ServerJuego.alto)?MapaServer.mapaFisico[quad_y+1][quad_x+1]:1;
        int jugador_centro=0;//MapaServer.jugadoresDibujados[quad_y][quad_x];//es el neo de jugador que esta bloqueando la pelota, si es 0 es por que no hay.
        
        /*
        if(((quad_x-1>0 && quad_y-1>0)?MapaServer.jugadores[quad_y-1][quad_x-1]:0)!=0)espacios[0][0]=2;
        if(((quad_y-1>0)?MapaServer.jugadores[quad_y-1][quad_x]:0)!=0)espacios[0][1]=2;
        if(((quad_x+1<ServerJuego.ancho && quad_y-1>0)?MapaServer.jugadores[quad_y-1][quad_x+1]:0)!=0)espacios[0][2]=2;
        if(((quad_x-1>0)?MapaServer.jugadores[quad_y][quad_x-1]:0)!=0)espacios[1][0]=2;
        if((MapaServer.jugadores[quad_y][quad_x])!=0)espacios[1][1]=2;
        if(((quad_x+1<ServerJuego.ancho)?MapaServer.jugadores[quad_y][quad_x+1]:0)!=0)espacios[1][2]=2;
        if(((quad_x-1>0 && quad_y+1<ServerJuego.alto)?MapaServer.jugadores[quad_y+1][quad_x-1]:0)!=0)espacios[2][0]=2;
        if(((quad_y+1<ServerJuego.alto)?MapaServer.jugadores[quad_y+1][quad_x]:0)!=0)espacios[2][1]=2;
        if(((quad_x+1<ServerJuego.ancho && quad_y+1<ServerJuego.alto)?MapaServer.jugadores[quad_y+1][quad_x+1]:0)!=0)espacios[2][2]=2;
        /*
